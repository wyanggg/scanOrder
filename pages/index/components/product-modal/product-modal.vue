<template>
  ...
  >
    <view class="header">
      ...
      <image
        src="/static/images/index/round_close_btn.png"
        @tap="$emit('cancel')"
      ></image>
    </view>
    <swiper
      ...
      <view class="wrapper">
        ...
        >
          ...
          <view class="values">
            <view
              class="value"
              :class="{ selected: material.select }"
              v-for="(material, index) in item.v"
              :key="index"
              @tap="changeMaterialSelected(item.k_s, material.id)"
            >
              {{ material.name }}
            </view>
          </view>
    ...
    <view
      class="bottom"
      :style="{ height: !is_single ? '250rpx' : '200rpx' }"
    >
