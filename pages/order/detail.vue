  ...
  <view class="container">
    ...
    >
      <view class="order-box">
        <image
          src="/static/images/order/icon_making.png"
          class="status-icon"
        />
        <view class="text-color-primary font-size-lg font-weight-bold mb-10">
          <template v-if="order.order.state == 0"> 订单未完成 </template>
          <template v-if="order.order.state == -1"> 订单退款中... </template>
          <template v-if="order.order.state == -2"> 订单退款已完成 </template>
          <template v-if="order.order.state == 1"> 订单已完成 </template>
        </view>
        <view
          class=""
          v-if="order.order.state == 1"
        >
      ...
      >
        <view class="order-info">
          <list-cell
            ...
            bgcolor="#FFFFFF"
          >
            <view class=""> 订单类型：{{ order.order.drive_type }}<br /> </view>
          </list-cell>
          <list-cell
            line-right
            bgcolor="#FFFFFF"
          >
            <view
              ...
              v-if="order.order.drive_type == '配送'"
            >
              ...
              <view class="d-flex align-items-center ml-20">
                <image
                  src="/static/images/order/icon_phone.png"
                  class="phone-icon"
                  @tap="call(order.order.receiver_mobile)"
                ></image>
              </view>
            </view>
            <view
              class=""
              v-if="order.order.drive_type == '店内就餐'"
            >
          ...
          <list-cell
            ...
            last
            bgcolor="#FFFFFF"
            style="border-radius: 0 0 30rpx 30rpx"
          >
          ...
          <list-cell
            v-if="order.order.state == 0 && order.order.payment_state == 1"
            padding="30rpx 40rpx"
            last
            bgcolor="#FFFFFF"
            style="border-radius: 0 0 30rpx 30rpx; text-align: right"
          >
            <button
              type="default"
              size="mini"
