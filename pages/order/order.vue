            ...
            <view class="order-list">
              <view class="order" v-for="(order, index) in orders" :key="index">
                ...
                <navigator open-type="navigate" :url="'/pages/order/detail?id=' + order.order_id">
                  <view class="images">
                    <image :src="order.ordergoods[0].pic"></image>
                ...
                <view class="action">
                  <view v-if="order.payment_state == 0" @tap="pay(order.order_id, order.payment_state)"
                    class="button button_pay" hover-class="none">去支付</view>
                  <view v-if="order.payment_state == 0" @tap="deleteOrder(order.order_id)" class="button button_del"
                    hover-class="none">删除</view>
                </view>
              </view>
            </view>
          </view>
                    ...
                    <view class="order" v-for="(order, index) in orders" :key="index">
                      <view class="header">
                        ...
                        <view class="status">
                          ...
                          <image @click="jump_detail(order.order_id)" src="/static/images/common/black_arrow_right.png">
                          </image>
                        </view>
                      </view>
                      <navigator open-type="navigate" :url="'/pages/order/detail?id=' + order.order_id">
                      ...
                      <view class="action" v-if="order.state == 1">
                        <view @tap="zlyd()" v-if="order.state != 0" class="button button_re" hover-class="none">再来一单
                        </view>
                      </view>
                    </view>
                  </view>
